<?php 
    ...
    $boardSql = "SELECT * FROM FBoard WHERE fDelete = 1 AND memberID = '$memberID' ORDER BY blogId DESC";
    ...
    // 총 페이지 갯수
    $sql = "SELECT count(blogId) FROM FBoard WHERE fDelete = 1 AND memberID = '$memberID'";
...
<?php
    ...
    // 이전 페이지 링크
    if($page > 1){
        echo "<li class='prev'><a href='myboard.php?page=".($page-1)."'>이전</a></li>";
    }
    
    // 다음 페이지 링크
    if($page < $boardTotalCount){
        echo "<li class='next'><a href='myboard.php?page=".($page+1)."'>다음</a></li>";
    }
?>
                    </ul>
                </div>
                <div class="board__table">
                    <table>
                        ...
                        <tbody>
<?php 
    $sql = "SELECT * FROM FBoard WHERE fDelete = 1 AND memberID = '$memberID' ORDER BY blogId DESC LIMIT {$viewLimit}, {$viewNum}";
    $result = $connect -> query($sql);
    ...
    if($sql){
        ...
        if($count > 0){
            for($i=0; $i<$count; $i++){
                ...
                echo "</td>";
                echo    "<td colspan='5' class='fL'><a href='../board/boardView.php?blogId={$info['blogId']}&nTitle={$info['fTitle']}&category={$info['fCategory']}'>{$info['fTitle']}</a></td>";
                echo "</tr>";
                echo "<tr class='fG'>";
                echo    "<td>";
